...
const socialConfig: Record<string, { icon: string; label: string; labelAr: string }> = {
  ...
  facebook: { icon: 'fa-facebook-f', label: 'Facebook', labelAr: 'فيسبوك' },
  twitter: { icon: 'fa-x-twitter', label: 'X', labelAr: 'تويتر' },
};
...
export default function Footer() {
  ...
  return (
    <footer style={{ background: '#111111', borderTop: '1px solid rgba(255,255,255,0.06)', padding: '32px 20px 24px' }}>
      <div style={{ maxWidth: 640, margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 28 }}>

        {/* Brand */}
        <div style={{ textAlign: 'center' }}>
          <p style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 900, fontSize: 32, color: '#ffffff', lineHeight: 1, marginBottom: 6 }}>
            {brand?.name || 'زيّ'}
          </p>
          <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', letterSpacing: '0.25em', textTransform: 'uppercase', fontFamily: 'Cairo, sans-serif' }}>
            {isRTL ? (brand?.taglineAr || 'أناقة كلاسيكية') : (brand?.taglineEn || 'Classic Elegance')}
          </p>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
          {quickLinks.map(link => (
            <div key={link.href} className="tooltip-wrap">
              <span className="tooltip-label">{isRTL ? link.labelAr : link.label}</span>
              <Link
                href={link.href}
                style={{
        ...
        {/* Copyright */}
        <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.2)', textAlign: 'center', fontFamily: 'Cairo, sans-serif' }}>
          {isRTL
            ? `© ${new Date().getFullYear()} ${brand?.name || 'زيّ'} — جميع الحقوق محفوظة`
            : `© ${new Date().getFullYear()} ${brand?.nameEn || 'Zayy'} — All rights reserved`}
        </p>
