...
export default function MenPage() {
  ...
  // Coming Soon check
  if (!loading && brand?.menComingSoon) {
    return (
      <PageTransition>
        <div style={{ minHeight: '100vh', background: 'var(--paper)' }}>
          ...
          <div style={{
            ...
            minHeight: '80vh', gap: 20, padding: 20,
          }}>
            <motion.div
            ...
            >
              <div style={{
                ...
                <i className="fa-solid fa-lock" style={{ fontSize: 32, color: 'var(--mid)' }} />
              </div>
              <h1 style={{ fontFamily: 'Cairo', fontWeight: 900, fontSize: 32, color: 'var(--ink)', marginBottom: 12 }}>
                {isRTL
                  ? (brand?.menComingSoonMessageAr || 'قريباً')
                  : (brand?.menComingSoonMessageEn || 'Coming Soon')}
              </h1>
              <p style={{ fontFamily: 'Cairo', fontSize: 14, color: 'var(--mid)' }}>
                {isRTL ? 'هذا القسم غير متاح حالياً' : 'This section is not available yet'}
              </p>
            </motion.div>
          </div>
          <Footer />
    ...
    <PageTransition>
      <div style={{ minHeight: '100vh', background: 'var(--paper)' }}>
        ...
        <section style={{
          ...
          <div
            style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', gap: 8, overflowX: 'auto' }}
          ...
          {loading ? (
            ...
            <div className="product-grid">
              {filtered.map((product, i) => (
                <ProductCard key={product.id} product={product} index={i} isRTL={isRTL} />
              ))}
            </div>
          )}
        </section>
  ...
  return (
    ...
    >
      <Link href={`/product/${product.id}`} style={{
        ...
        overflow: 'hidden', background: 'var(--paper-soft)', aspectRatio: '3/4',
      }}>
        {hasImage ? (
          <Image
